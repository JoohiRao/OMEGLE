...
const socketIO = require('socket.io');
 const server = http.createServer(app); //creating a server of using http which will use the functionality of app
...
//all your details are in socket
 io.on("connection" , function (socket) { // io ko recieve kiya h front end se isliye on use kr rhe h nd receive kiya h connection pr 
    ...
    socket.on("joinroom", function () {
        ...
        if(waitingUsers.length > 0 ){
           ...
           // When a user joins a room, the backend emits a message to notify all users in that room about the event.
       ...
       socket.on("message", function (data) {
        console.log(data)  // this give rrom and msg
